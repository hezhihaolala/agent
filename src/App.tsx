...
import type { AgentResult, AnswerResult, AuditLog, DraftPreview, Person, Relationship, Source } from './types'
import './styles.css'
...
function Assistant({ memberCount, refresh }: { memberCount: number; refresh: () => Promise<void> }) {
  ...
  const draft = result?.type === 'draft' ? result : null, answer = result && result.type !== 'draft' ? result : null
...
function EvidencePanel({ answer }: { answer: AnswerResult | null }) { const relationships = answer ? (answer.type === 'relative_list' ? answer.relationships : [answer.relationship]) : []; return <div className="context-content"><p className="context-kicker">{answer ? '本次回答依据' : '智能体工作台'}</p><h2>{answer ? '关系路径' : '可信回答，有据可查'}</h2>{answer ? <>{relationships.map((relationship, pathIndex) => <div className="path-stack" key={`${relationship.label}-${pathIndex}`}><strong className="path-label">{relationship.label}</strong>{relationship.steps.map((step, index) => <div key={`${step.person_id}-${index}`}>{index > 0 && <div className="relation-line"><i /><span>关系链第 {index} 步</span></div>}<PersonLine name={step.person_name} /></div>)}</div>)}{answer.sources.map((source) => <div className="source-card" key={source.id}><span>{source.verification_status === 'verified' ? '已核实来源' : '待核实来源'}</span><strong>{source.title}</strong><a href={`/api/sources/${source.id}/download`}>查看原始档案 ↗</a></div>)}{!answer.sources.length && <Empty text="尚未关联资料，此结论需要补充证据。" />}</> : <div className="empty-context"><div>引</div><p>提问后，这里会展示关系路径、引用资料和核实状态。</p></div>}</div> }
...
function FamilyTree({ people, relationships, refresh, onAdd, onEdit }: { people: Person[]; relationships: Relationship[]; refresh: () => Promise<void>; onAdd: () => void; onEdit: (person: Person) => void }) {
  const [kind, setKind] = useState<Relationship['kind']>('parent')
  ...
  return <main className="page simple-page"><div className="page-heading"><div><p className="eyebrow">GENEALOGY</p><h1>族谱与人物档案</h1><p>{relationships.length} 条正式关系，点击人物可编辑档案。</p></div><button className="primary-small" onClick={onAdd}>＋ 添加成员</button></div>
    ...
    <section className="panel relationship-manager"><div className="panel-title"><div><span>关系维护</span><h2>亲属关系</h2></div></div><form onSubmit={create}><label>关系类型<select value={kind} onChange={(event) => setKind(event.target.value as Relationship['kind'])}><option value="parent">父母关系</option><option value="spouse">配偶关系</option><option value="sibling">兄弟姊妹关系</option><option value="paternal_cousin">堂兄弟姊妹关系</option></select></label><label>人物<select aria-label="人物" value={personId} onChange={(event) => setPersonId(event.target.value)}>{people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label><label>亲属<select aria-label="亲属" value={relativeId} onChange={(event) => setRelativeId(event.target.value)}>{people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label><button className="primary-small" disabled={people.length < 2}>建立关系</button></form>{error && <p className="form-error" role="alert">{error}</p>}<div className="relationship-list">{relationships.map((item) => <div key={item.id}><span>{relationshipDescription(item, personMap)}</span><button type="button" onClick={() => void remove(item.id)}>删除</button></div>)}{!relationships.length && <Empty text="还没有亲属关系。" />}</div></section>
  </main>
}
function relationshipDescription(item: Relationship, people: Map<string, string>) {
  const person = people.get(item.person_id), relative = people.get(item.relative_id)
  if (item.kind === 'parent') return `${person} 的父母是 ${relative}`
  if (item.kind === 'spouse') return `${person} 的配偶是 ${relative}`
  return `${person} 与 ${relative} 是${item.kind === 'sibling' ? '兄弟姊妹' : '堂兄弟姊妹'}`
}
...
function Settings({ username, onLogout }: { username: string; onLogout: () => Promise<void> }) {
  ...
  const changePassword = async (event: FormEvent) => {
    ...
    setBusy(true)
    try { await api.changePassword(currentPassword, newPassword); setCurrentPassword(''); setNewPassword(''); setConfirmation(''); setMessage('密码已更新') }
    catch (reason) { setError(reason instanceof Error ? reason.message : '密码修改失败') }
    finally { setBusy(false) }
  }
  return <main className="page simple-page"><div className="page-heading"><div><p className="eyebrow">SYSTEM</p><h1>设置</h1><p>敏感配置由服务器环境变量管理，不会发送到浏览器。</p></div></div><div className="panel settings-panel"><h2>安全与模型</h2><div className="setting-row"><div><strong>当前管理员</strong><p>{username} · 单管理员模式</p></div><button className="outline-button" onClick={() => void onLogout()}>退出登录</button></div><form className="password-form" onSubmit={changePassword}><h3>修改管理员密码</h3><label>当前密码<input aria-label="当前密码" type="password" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} required /></label><label>新密码<input aria-label="新密码" type="password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required minLength={12} /></label><label>确认新密码<input aria-label="确认新密码" type="password" autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} required minLength={12} /></label>{error && <p className="form-error" role="alert">{error}</p>}{message && <p className="form-success" role="status">{message}</p>}<button className="primary-small" disabled={busy}>{busy ? '正在更新…' : '修改密码'}</button></form><div className="setting-row"><div><strong>写入前人工确认</strong><p>智能体正式写入只能通过已保存草稿的确认接口。</p></div><span className="toggle on"><i /></span></div><div className="setting-row"><div><strong>托管模型服务</strong><p>OpenAI 兼容 API · 由服务端配置</p></div><span className="toggle on"><i /></span></div></div></main>
}
