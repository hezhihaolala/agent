...
面向单管理员的中文族谱 Web 应用。它可以维护人物、父母/配偶/兄弟姊妹/堂兄弟姊妹关系和私密档案，通过确定性规则回答亲属关系，并让托管模型把自然语言整理为待确认草稿。模型不能直接写正式数据。

## 已实现

- 单管理员登录、服务端会话和 CSRF 防护。
- 人物增删改查、父母/配偶/兄弟姊妹/堂兄弟姊妹关系维护、重复与祖先循环校验。
- 确定性亲属路径和常见中文称谓。
- PDF、图片、文字资料私密上传、哈希校验、鉴权下载和证据关联。
- 常见“某人的父母/子女/配偶/兄弟姊妹/堂兄弟姊妹是谁”查询由后端直接解析，无需调用模型。
